...
uses Classes, ComCtrls, //ExtListView,

     // TExportExtListView Units
     EC_Main;
...
type
  ...
  TExportExtListView = class(TSharewareExportComponent)
  private
//    FListView: TExtListView;
    ...
    // *** Set Access Methods ***
//    procedure SetExtListView(Value: TExtListView);
  ...
  published
//    property ListView: TExtListView read FListView write SetExtListView;
...
constructor TExportExtListView.Create(AOwner: TComponent);
var
  i: Integer;
begin
  inherited;
...
{  if not Assigned(FListView) then
  with TForm(Owner) do
    for i := 0 to ComponentCount - 1 do
      if Components[i] is TExtListView then
      begin
        FListView := TExtListView(Components[i]);
        break;
      end;
}
end;

procedure TExportExtListView.InitExport;
begin
  inherited;
...
procedure TExportExtListView.CleanUpExport;
begin
  inherited;
...
function TExportExtListView.GetNumExportableItems: Integer;
begin
{  with FListView do
...
function TExportExtListView.CurrentItemSelected: Boolean;
begin
//  Result := FListView.Items[FCount.CurrentRow].Selected;
end;

function TExportExtListView.ColAllowed(ListCol: TListColumn): Boolean; // Returns True if the Column should be exported
begin
...
function TExportExtListView.GetColIndex(aIndex: Integer): Integer;
begin
...
function TExportExtListView.GetColValue(aIndex: Integer): String;
begin
//  Result := FListView.Column[aIndex].Caption;
end;

function TExportExtListView.GetExportColumns(var aColumns: TStringList): TStringList;
  function GeTExtListViewColumnOrder: Pointer;
  const
    LVM_GeTExtListViewColumnOrderARRAY = LVM_FIRST + 59;
  begin
{    with FListView do
    try
      ...
      if SendMessage(Handle, LVM_GeTExtListViewColumnOrderARRAY, Columns.Count, LPARAM(Result)) = 0 then
      begin
...
{    ColOrderArray := GeTExtListViewColumnOrder; // Get the Order of the columns (it may not be as listed in the
...
procedure TExportExtListView.GetExportItemData(var aExportItem: TStringList);
var
 j, k: Integer;
begin
  // Allocate StringList Capacity
...
function TExportExtListView.HasData: Boolean;
begin
  Result := True; // Assume Success
...
{  if Assigned(FListView) then
  ...
  else raise Exception.CreateFmt(cNoComponentError, ['TExtListView']);
}
end;

// *** Set Access Methods ***

{procedure TExportExtListView.SetExtListView(Value: TExtListView);
begin
...
procedure TExportExtListView.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited;
